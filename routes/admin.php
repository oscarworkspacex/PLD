...
// Rutas de Excel - SIN middleware de permisos para evitar conflictos
// La autorización se maneja en el frontend
Route::middleware(["auth", "verified"])->group(function () {
    Route::get("excel", [ExcelController::class, "index"])
        ->name("admin.excel.index");
    
    Route::get("search-data", [ExcelController::class, "searchData"])
        ->name("admin.excel.search.data");
    
    Route::get("excel/files", [ExcelController::class, "files"])
        ->name("admin.excel.files");
    
    Route::get("excel/file-preview", [ExcelController::class, "preview"])
        ->name("admin.excel.preview");
    
    Route::get("excel/file-preview/rows", [ExcelController::class, "previewRows"])
        ->name("admin.excel.preview.rows");
    
    Route::get("excel/list", [ExcelController::class, "listFiles"])
        ->name("admin.excel.list");

    Route::post("excel/upload", [ExcelController::class, "upload"])
        ->name("admin.excel.upload");

    Route::delete("excel/delete", [ExcelController::class, "deleteFile"])
        ->name("admin.excel.delete");
});
...
Route::middleware(["auth", "verified"])->group(function () {
    Route::get("alertas-lista-negra", [\App\Http\Controllers\Admin\AlertaListaNegraController::class, "index"])
        ->name("admin.alertas-lista-negra.index");
    Route::patch("alertas-lista-negra/{alerta}/leida", [\App\Http\Controllers\Admin\AlertaListaNegraController::class, "markAsRead"])
        ->name("admin.alertas-lista-negra.mark-read");
    Route::delete("alertas-lista-negra/{alerta}", [\App\Http\Controllers\Admin\AlertaListaNegraController::class, "destroy"])
        ->name("admin.alertas-lista-negra.destroy");
});
